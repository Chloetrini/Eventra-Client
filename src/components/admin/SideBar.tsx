...
// label above it ( "Overview" is its own row, not
...
function NavCountBadge({ count, urgent }: { count: number; urgent?: boolean }) {
    ...
    return (
        <span
            className={`flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[11px] font-bold ${urgent
...
const SideBar: React.FC<AdminSideBarProps> = ({ isOpen = false, onClose }) => {
    ...
    // NOTE: assumes useAuth() exposes isLoading alongside user — adjust the
    // field name here if your context calls it something else.
    const { user, isLoading: isAuthLoading } = useAuth();
    // Placeholder counts for the Needs Action badges — pending a real
    // "admin nav counts" endpoint. See hooks/useAdminNavCounts.ts.
    const { data: counts, isLoading: isCountsLoading } = useAdminNavCounts();

    // Gate on both — user info (name/role/avatar) and the badge counts are
    // both needed for the nav to render as intended, so show one skeleton
    // until both are ready rather than popping badges in a beat after the
    // rest of the sidebar.
    if (isAuthLoading || isCountsLoading) return <SideBarSkeleton />;
    ...
    const renderItem = (item: {
        ...
        // `counts` is typed as AdminNavCounts | undefined straight from the
        // hook, and countKey is constrained to keyof AdminNavCounts above —
        // so this indexes cleanly with no cast needed.
    ...
    return (
        <>
            ...
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-64 bg-card border-r border-border flex flex-col h-screen transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
