...
const SideBar: React.FC<AdminSideBarProps> = ({ isOpen = false, onClose }) => {
  ...
  // "admin nav counts" endpoint. See hooks/use-admin-nav-counts.ts.
  ...
  const renderItem = (item: {
    ...
    // ✅ PERFECT TYPE FIX: Create a strictly typed version of counts
    const typedCounts = counts as Record<'pendingApprovals' | 'pendingRefunds' | 'flaggedReports', number | undefined>;
    
    const count = item.countKey ? typedCounts?.[item.countKey] : undefined;
...
// // nav item itself rather than something you only see after clicking in.
// const needsActionItems = [
//   { icon: ClipboardCheck, label: 'Approvals', path: '/admin/approvals', countKey: 'approvals' as const },
//   { icon: Undo2, label: 'Refunds', path: '/admin/refunds', countKey: 'refunds' as const },
//   { icon: FileWarning, label: 'Reports', path: '/admin/reports', countKey: 'reports' as const, urgent: true },
// ];
...
//     label: string;
//     path: string;
//     countKey?: 'approvals' | 'refunds' | 'reports';
//     urgent?: boolean;
//   }) => {
//     const active = isActive(item.path);
