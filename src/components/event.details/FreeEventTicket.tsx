...
export const FreeEventTicket = ({
  ...
  const spotsLeft = hasCapLimit
    ? Math.max(capacity - reservationsCount, 0)
    : null;
  const percentFilled =
    hasCapLimit && capacity > 0
      ? ((capacity - (spotsLeft ?? 0)) / capacity) * 100
      : 0;
  const isFull = hasCapLimit && (spotsLeft ?? 0) <= 0;

  const increment = () =>
    setGuests((g) => Math.min(MAX_GUESTS_PER_RESERVATION, g + 1));
  ...
  const handleReserve = () => {
    ...
    navigate("/payment/checkout", {
      state: {
        ...
        ticketDetails: [
          { id: 1, type: "Free", unitPrice: 0, quantity: guests },
        ],
        slug: event.slug,
        guests,
      },
  ...
  return (
    <div className="rounded-2xl border p-6 shadow-[0_0_15px_rgba(0,0,0,0.15)] h-fit">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-extrabold text-[#1A1523]">
          Reserve your spot
        </h2>
        <span className="rounded-full w-22 h-8 flex items-center justify-center bg-[#FCEBC9] px-3 py-1 text-sm font-medium text-[#7A4E02]">
          Free
        </span>
      </div>
        ...
        <div className="mt-6 space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-bold">
              {spotsLeft}{" "}
              <span className="font-normal text-[#6E6577]">/ {capacity}</span>
            </span>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Up to {MAX_GUESTS_PER_RESERVATION} per reservation
            </p>
          </div>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="font-bold tracking-wider font-space text-[#0F6E56]">
          Free
        </span>
      </div>
      <p className="mt-2 text-xs font-normal text-[#4A4451]">
        No payment needed. You'll get a QR ticket to show at the door and you
        can cancel anytime to release your spot.
      </p>
      ...
      <div className="mt-4 space-y-1.5">
        ...
        <p className="flex items-center gap-1.5 text-xs text-[#6E6577]">
          <Mail className="h-3.5 w-3.5" /> Instant confirmation sent to your
          mail
        </p>
      </div>
    </div>
