...
const navItems = [
  ...
  { icon: Users, label: "Attendees", path: "/dashboard/attendees" },
  {
    icon: CheckSquare,
    label: "Check-in",
    path: "/organizer/dashboard/checkin",
  },
  { icon: Wallet, label: "Payouts", path: "/organizer/dashboard/payouts" },
  {
    icon: Megaphone,
    label: "Promotions",
    path: "/organizer/dashboard/promotions",
  },
];
    ...
    <aside className="w-64 bg-white border-r border-[#EFEEED] flex flex-col h-screen">
      ...
      <div className="flex items-center gap-3 px-6 pt-6 pb-4">
        <div className="flex flex-row items-center">
          <img
            src={UI_ASSETS.Eventraa}
            onClick={() => navigate("/")}
            alt="Eventra-Logo"
          />
          <span className="font-extrabold text-[27px] dark:text-white font-grotesk tracking-tight">
            Eventra
          </span>
        </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <item.icon
                  className={`h-5 w-5 ${isActive ? "text-[#0F6E56]" : "text-[#6E6577]"}`}
                />
                {item.label}
              </div>
              {item.label === "Attendees" && (
                <span className="bg-[#F59E0B] text-white text-[10px] font-bold h-5 w-5 rounded-full flex items-center justify-center">
                  3
                </span>
              )}
            </Link>
