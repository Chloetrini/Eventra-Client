...
export default function About() {
    ...
    return (
        <PageWrapper className="p-[20px]">
            ...
            <main className=" overflow-x-hidden bg-[#FFFFFF] antialiased">
                <section className="flex flex-col md:flex-row lg:flex-row items-center justify-between gap-6 lg:gap-15 ">
                    <div className="w-full lg:w-1/2 flex flex-col gap-4 md:gap-5 lg:items-start">
                        <div className="flex items-center gap-2 md:gap-4">
                            <span className="border-2 w-3 md:w-8 md:h-[3px] border-[#F5A524] md:border-[#0F6E56]"></span>
                            <p className="font-schibsted font-semibold text-sm md:text-2xl text-[#0F6E56]">
                                About Eventra
                            </p>
                        </div>

                        <div className="flex flex-col gap-4 md:gap-6">
                            <h1 className="font-geist md:font-schibsted font-bold md:font-extrabold text-[30px] sm:text-[40px] md:text-[30px] lg:text-[40px] xl:text-[50px] 2xl:text-[50px] leading-tight text-[#000000] break-words">
                                We’re fixing how Nigeria buys tickets.
                            </h1>
                            <p className="font-geist font-normal md:font-medium text-md md:text-sm lg:text-md xl:text-[20px] leading-relaxed text-[#6E6577] break-words">
                ...
                <section className="w-full flex flex-col md:flex-row items-center justify-between gap-8 md:gap-10 xl:gap-12  md:my-12">
                    <div className="order-1 md:order-2 w-full md:max-w-xl flex flex-col gap-4 md:gap-3">
                        <h1 className="font-geist md:font-schibsted font-bold md:font-extrabold text-3xl sm:text-[40px] md:text-[25px] lg:text-[30px] xl:text-[35px] leading-tight text-black break-words">
                            Build for the way lagos actually parties.
                        </h1>
                        <div className="flex flex-col gap-3 md:gap-2 text-[14px] sm:text-[19px] md:text-[13px] lg:text-[18px] leading-relaxed text-[#6E6577] font-geist font-normal md:font-medium break-words">
                            <p>
                ...
                <section className=" my-12">
                    <div className="flex flex-col gap-6 md:gap-10">
                        <div className="w-full md:max-w-md flex flex-col gap-1 md:gap-2">
                            <p className="font-geist font-medium text-sm sm:text-base md:text-lg text-[#0F6E56]">
                                WHAT WE BELIEVE
                            </p>
                            <h1 className="font-geist font-bold text-xl sm:text-2xl md:text-3xl lg:text-4xl tracking-wide text-[#1A1523]">
                                Our principles
                            </h1>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
                            {principles.map((item, index) => (
                                <div
                                    key={index}
                                    className="rounded-lg border border-[#E8E6E0] p-5 md:p-7 flex flex-col gap-4 bg-white shadow-sm hover:shadow-md transition-shadow"
                                >
                                    <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-[#E4F1EB] flex items-center justify-center text-[#0F6E56]">
                                        {item.icon}
                                    </div>

                                    <h2 className="font-geist md:font-schibsted font-bold text-lg sm:text-xl md:text-2xl text-black">
                                        {item.title}
                                    </h2>

                                    <p className="font-geist font-normal text-sm sm:text-base md:text-lg leading-relaxed text-[#4A4451]">
                                        {item.description}
                                    </p>
                                </div>
                            ))}
                ...
                <section className="w-full px-4 sm:px-6 md:px-10 lg:px-16 xl:px-20 my-12">
                    <div className="flex flex-col gap-6 md:gap-10 rounded-lg p-6 md:p-10 bg-[#FFFFFF]">
                        {rows.map((row, rowIndex) => (
                            ...
                            >
                                {row.map((item, index) => (
                                    ...
                                    >
                                        <div className="w-[89px] h-[89px] md:w-14 md:h-14 rounded-full bg-[#E4F1EB] flex items-center justify-center text-[#0F6E56]">
                                            {item.icon}
                                        </div>
                                        <div className="flex flex-col gap-1 md:gap-2 items-center">

                                        <h2 className="font-geist font-bold  sm:text-xl md:text-[34px] text-black font-grotesk">
                                            {item.currency ? `${item.currency} ${item.value}` : item.value}
                                        </h2>

                                        <p className="font-geist text-sm sm:text-base md:text-lg text-[#4A4451]">
                                            {item.label}
                                        </p>
                                        </div>
                                    </div>
